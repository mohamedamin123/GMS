		...
		bt3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					st.executeUpdate("update client set telephone='"+t4.getText()+"',nom='"+t1.getText()+"',idC='"+t2.getText()+"',email='"+t3.getText()+"'where nom='"+tab.getValueAt(i, 0)+"'");
...
public void supprimer()
{
    ...
    try {
		...
		Connection db=DriverManager.getConnection("jdbc:mysql://localhost/dbstock", "root", "");
		Statement st=db.createStatement();
		st.executeUpdate("delete from client where 	idC='"+tab.getValueAt(i, 1)+"'");
